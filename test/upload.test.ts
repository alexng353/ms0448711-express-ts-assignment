...
describe("POST /upload", () => {
  it("should return {message:'upload complete',success:true}", async () => {
    const pdfDoc = path.resolve(
      __dirname + "/../data/Business+Backgrounder+B+.pdf",
    );
    ...
    expect(response.text).toBe(
      JSON.stringify({ message: "upload complete", success: true }),
    );
  });

  it("should fail because of size limit", async () => {
    const pdfDoc = path.resolve(
      __dirname + "/../data/CAHSMUN+2021+Delegate+Handbook.pdf",
    );
    const response = await request(app)
      .post("/upload")
      .attach("pdfDoc", pdfDoc);
  ...
  it(
    "should run eleven times and return 'Too Many Request'",
    async () => {
      const pdfDoc = path.resolve(
        __dirname + "/../data/Business+Backgrounder+B+.pdf",
      );
